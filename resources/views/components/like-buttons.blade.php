<div class="flex mx-2">
    <form action="/tweets/{{$tweet->id}}/like" method="POST">
        ...
        <div class="flex items-center {{$tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500'}}">
            <button type="submit"><svg viewBox="0 0 20 20" class="w-3 mr-2" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <form action="/tweets/{{$tweet->id}}/like" method="POST">
            @csrf
            @method('DELETE')
            <div class="flex items-center ml-2 ml-4 {{$tweet->isDisLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500'}}">
              <button type="submit">  <svg viewBox="0 0 20 20" version="1.1" class=" w-3 mr-2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g class="fill-current">
                ...
                <button type="submit">{{$tweet->dislikes ?: 0}}</button>
            </div>
